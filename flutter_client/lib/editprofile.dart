...
//           String photoData = jsonDecode(response.body)['photo'];
//           String photo = (photoData != null && photoData.isNotEmpty)
//               ? img_url + photoData
...
class _EditProfileState extends State<EditProfile> {
  ...
  void _edit_data() async {
    ...
    if (uphone.isEmpty || !RegExp(r'^[0-9]{10}$').hasMatch(uphone)) {
      Fluttertoast.showToast(msg: 'Please enter a valid 10-digit phone number');
      return;
    }

    if (uemail.isEmpty || !RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(uemail)) {
    ...
    if (uplace.isEmpty) {
      Fluttertoast.showToast(msg: 'Please enter your place');
      return;
    }

    if (upost.isEmpty) {
      Fluttertoast.showToast(msg: 'Please enter your post');
      return;
    }

    if (upin.isEmpty || !RegExp(r'^[0-9]{6}$').hasMatch(upin)) {
      Fluttertoast.showToast(msg: 'Please enter a valid 6-digit PIN');
      return;
    }

    if (udistrict.isEmpty) {
      Fluttertoast.showToast(msg: 'Please enter your district');
      return;
    }

    if (ueventdate.isEmpty) {
